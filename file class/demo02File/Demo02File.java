...
public class Demo02File {
     ...
     * 参数:把路径分成了两部分 File parent:父路径 String child:子路径 好处:
    ...
    private static void show03() {
        ...
        System.out.println(file);// 重写toString方法
    }

    /*
     * File(String parent, String child) 根据 parent 路径名字符串和 child 路径名字符串创建一个新 File
     * 实例。 参数:把路径分成了两部分 String parent:父路径 String child:子路径 好处:
     * 父路径和子路径,可以单独书写,使用起来非常灵活;父路径和子路径都可以变化
     */
    ...
    /*
     * File(String pathname) 通过将给定路径名字符串转换为抽象路径名来创建一个新 File 实例。 参数: String
     * pathname:字符串的路径名称 路径可以是以文件结尾,也可以是以文件夹结尾 路径可以是相对路径,也可以是绝对路径 路径可以是存在,也可以是不存在
    ...
    private static void show01() {
        ...
        System.out.println(f2);// 重写toString方法

        File f3 = new File("a.txt");
        System.out.println(f3);// 重写toString方法
    }
}
